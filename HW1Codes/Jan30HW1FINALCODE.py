...
for k in range(0,len(problem3_time_list)):
	...
	plt.plot(g_r_color_array[j], mag_diff_time_array[j], linestyle='-', \
			marker=time_marker_list[k], markersize=time_markersize_list[k],  label=str(problem3_time_list[k]) )
...
plt.xlabel(x_axis4)
plt.ylabel(y_axis4)
plt.legend(title='Ages [Gyrs]', loc='best')
