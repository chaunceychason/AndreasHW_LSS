...
for i in range(0, len(RADECZdata)):
	...
	#print x1, y1, z1 
	for j in range(i, len(RADECZdata)):
		if i != j:
...
for i in range(0, len(RADECZ2data)):
	...
	#print x1, y1, z1 
	for j in range(i, len(RADECZ2data)):
		if i != j:
...
for i in range(0, len(RADECZranddata) ):
	...
	#print x1, y1, z1 
	for j in range(i, len(RADECZranddata)):
		if i != j:
...
for i in range(0, 15):
	#Xi[i] = ((len(RADECZdata)/len(RADECZranddata))**2) * ( logr_datacountlist[i]/logr_randcountlist[i] - 1.0)
	Xi2[i] = ((len(RADECZ2data) / len(RADECZranddata))**2) * ( logr_data21countlist[i]/logr_randcountlist[i] - 1.0)
