...
int main ()  
{
   ...
   */                      
   ...
   printf("FIINISHED Mr20 NESTED LOOP. \n");
   printf("Counts: ");

   for(i =0; i< num_bins; ++i){
      printf("%d ", distance20_counts[i]);
   }
   printf("\n") ;  
   ...
   static const char r20z_datafile[] = "SDSS_Mr20z_rspace.dat"; 
   ...
   */
   ...
   for(i =0; i< num_bins; ++i){
      printf("%d ", randdistance_counts[i]);
   ...
   for(i=0; i<num_bins; i++){
      ...
      Xi_func[i] = (N_data / N_rand)*(N_data / N_rand) * \
   ...
   for(i=0; i<num_bins; i++){
      //Compute the Correlation Function: Xi = (Ngal/Nrand)^2 * (DD/RR  - 1)
      Xi20_func[i] = (N20_data / N_rand)*(N20_data / N_rand) * \
   ...
   for(i=0; i<num_bins; i++){
      //Compute the Correlation Function: Xi = (Ngal/Nrand)^2 * (DD/RR  - 1)
      Xi20z_func[i] = (N20z_data / N_rand)*(N20z_data / N_rand) * \
